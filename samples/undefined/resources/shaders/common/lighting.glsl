...
vec3 CalcDirectionalLight(in FragProperties fragProperties, in Light light, in Material material) {
    // Ambient lighting
    vec3 ambient = light.color * light.ambientProperty * vec3(texture(material.diffuse, fragProperties.texCoords));

    // Diffuse lighting
    vec3 normal = normalize(fragProperties.normal);
    vec3 lightDirection = normalize(-light.direction);
    float diffuseValue = max(0.0f, dot(normal, lightDirection));
    vec3 diffuse = light.color * light.diffuseProperty * diffuseValue * vec3(texture(material.diffuse, fragProperties.texCoords));
...
vec3 CalcPointLight(in FragProperties fragProperties, in Light light, in Material material) {
    // Ambient lighting
    vec3 ambient = light.color * light.ambientProperty * texture(material.diffuse, fragProperties.texCoords).rgb;
    ...
    vec3 diffuse = light.color * light.diffuseProperty * diffuseValue * texture(material.diffuse, fragProperties.texCoords).rgb;
